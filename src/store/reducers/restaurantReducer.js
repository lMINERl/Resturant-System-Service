...
const restaurantReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case actions.UPDATE_ITEM:
            ...
            if (index !== -1)
                restaurants[index] = updatedItem;

            break;

        case actions.DELETE_ITEM:
            const _id = action.payload;
            const itemIndex = restaurants.findIndex(el => el.__id === _id);
            if (itemIndex !== -1)
                restaurants = [].concat(restaurants.slice(0, itemIndex), restaurants.slice(itemIndex, restaurants.length));
            break;

        case actions.GET_ITEM_BY_ID:
            const __id = action.payload;
            const itemindex = restaurants.findIndex(el => el.__id === __id);
            if (itemindex !== -1)
                selectedRestaurant = itemindex;
            break;
